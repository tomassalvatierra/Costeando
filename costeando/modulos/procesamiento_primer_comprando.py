...
def procesar_descuento(df_calculo_comprando, df_costos_especiales, campaña, año, df_compras):
    ...
    df_base_descuentos, df_cambios = actualizar_estado_vencido(df_sin_vencidos01, int(campaña), int(año), int(campaña) - 5)
...
def procesar_primer_comprando(campaña, año, indice_a, indice_b, mano_de_obra,
    ...
    try:
        ...
        lista_dfs = [
            (df_maestro, 'Maestro'), (df_compras, 'Compras'), (df_stock, 'Stock'),
            (df_costos_especiales, 'Base Especiales'), (df_listado_anterior, 'Listado'),
            (df_calculo_comprando_anterior, 'Comprando Anterior'), (df_ficha_rms, 'Ficha RMS')]
        ...
        for old, new in [('1','A'),('2','B'),(1,'A'),(2,'B')]:
            df_calculo_comprando['Cod Actualiz'] = df_calculo_comprando['Cod Actualiz'].replace(old, new)
        for old, new in [(1,'Si'),(2,'No'),('1','Si'),('2','No')]:
            df_calculo_comprando['Blq. de Pant'] = df_calculo_comprando['Blq. de Pant'].replace(old, new)
        df_calculo_comprando['¿Atiende Ne?'] = df_calculo_comprando['¿Atiende Ne?'].replace('P', 'Produciendo')
        df_calculo_comprando = df_calculo_comprando.drop(df_calculo_comprando[
            (df_calculo_comprando['Tipo'].isin(['GG','GN','GI'])) |
            (df_calculo_comprando['Blq. de Pant'] == 'Si') |
            (df_calculo_comprando['¿Atiende Ne?'] == 'Produciendo')].index)
        columns_to_merge = ['ULTCOS','Fch Emision','OBSERVACIONES COSTOS','RESPUESTA COMPRAS','Campaña','MONEDA',
                            'Stock Actual','COSTO LISTA '+año_campaña_anterior,'Costo sin Descuento C'+campaña_anterior]
        data_frames_to_merge = [df_compras,df_compras,df_compras,df_compras,df_compras,df_compras,
                                df_stock,df_listado_anterior,df_calculo_comprando_anterior]
        ...
        df_calculo_comprando, df_costos_especiales, df_no_vencidos, df_cambios = procesar_descuento(
            df_calculo_comprando, df_costos_especiales, campaña, año, df_compras)
        ...
        df_calculo_comprando = calcular_variacion(df_calculo_comprando, 'Costo Compra',
            'Costo sin Descuento C'+campaña_anterior, '% var Compra VS Costo sin dto C'+campaña_anterior)
        df_calculo_comprando = calcular_variacion(df_calculo_comprando, 'Costo Compra',
            'COSTO LISTA '+año_campaña_anterior, '%var Compra vs COSTO LISTA C'+campaña_anterior)
        df_calculo_comprando.loc[df_calculo_comprando['Costo Compra'].notnull(), 'Coef de Actualizacion'] = 1
        df_calculo_comprando['Costo sin Descuento C'+campaña] = df_calculo_comprando.apply(
            lambda row: calcular_costo_sin_descuento(campaña_anterior, row), axis=1)
        columnas_a_rellenar = ['DESCUENTO ESPECIAL','% de obsolescencia','ROYALTY',
                               'Costo sin Descuento C'+campaña,'Costo sin Descuento C'+campaña_anterior]
