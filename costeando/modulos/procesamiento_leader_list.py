...
def proceso_combinadas(df_combinadas: pd.DataFrame) -> pd.DataFrame:
    df_grouped = df_combinadas.groupby('COMBINADA')['CODIGON'].agg(
...
def procesar_leader_list_puro(
    ...
    try:
        ...
        (df_costo_anterior, df_maestro, df_stock, df_leader_list,
         df_dobles, df_combinadas_agrupadas, df_combinadas) = lista_dfs

        df_leader_list = df_leader_list.query(
            "Codigo not in [99000, 99001, 99002, 99003, 99004, 99005]"
        )

        columnas_a_incluir = [
            "COSTO LISTA " + anio_campana, "TIPO COSTO",
            "%VAR C" + campana + "VS C" + campana_anterior,
        ...
        if "DESCUENTO ESPECIAL" in df_costo_anterior.columns:
            df_leader_list = pd.merge(
                df_leader_list,
                df_costo_anterior[["Codigo", "DESCUENTO ESPECIAL", "APLICA DDE CA:"]],
                how="left", on="Codigo"
            )
            df_leader_list = pd.merge(
                df_leader_list,
                df_costo_anterior[["Codigo", "TIPO-DESCUENTO"]],
                how="left", on="Codigo"
            )
            df_leader_list.rename(columns={
        ...
        df_leader_list = pd.merge(
            df_leader_list, df_combinadas_agrupadas[["Codigo", "COD COMB"]], how="left"
        )
        df_leader_list.rename(columns={"Estado": "ESTADO TOTVS"}, inplace=True)
        df_leader_list = pd.merge(
            df_leader_list, df_stock[["Codigo", "Stock Actual"]], how="left"
        )
        ...
        columnas_finales = [
            ...
            "LINEA", "LEY_LIN", "PROC", "LEYEPRO", "UNID_EST", "CONSIG", "CABECERA",
            "COEFICIE", "EMPRESA", "ESTADO", "CMP_ESTA", "CUOTAS", "OM_EST",
            "PED_EST", "COS_ESTOT", "COSTO LISTA " + anio_campana_anterior,
            "%VAR C" + campana + " VS C" + campana_anterior,
            "VAR C" + campana + " VS CESTIM", "OBSERVACIONES",
            "DESCUENTO ESPECIAL (N-1)", "TIPO-DESCUENTO (N-1)", "APLICA DDE CA:",
            "ESTADO TOTVS", 'Stock Actual', 'UNIDADES REALES ESTIMADAS',
        ...
        path_leader_list = os.path.join(
            carpeta_guardado, f"{fecha_hoy} Leader List procesado C{campana}-{anio}.xlsx"
        )
        path_combinadas_agrupadas = os.path.join(
            carpeta_guardado, f"{fecha_hoy} Combinadas agrupadas C{campana}-{anio}.xlsx"
        )
