...
from costeando.utilidades.validaciones import validar_archivo_excel, validar_columnas, estandarizar_columna_producto
...
def actualizar_estado_vencido(df_base_dtos, campania_actual, anio_actual, campania_stock):
    ...
    cambios = (
        ...
        (df_base_dtos["NOTAS"] == "Pierde descuento por sobrepasar de 27 campañas(año y medio)") |
        (df_base_dtos["NOTAS"] == "Pierde descuento por sobrepasar de 18 campañas"))
...
def procesar_primer_produciendo(
    ...
    try:
        ...
        df_produciendo["Costo sin Descuento C"+str(campaña_actual)] = df_produciendo.apply(
        ...
        df_sin_vencidos, df_no_vencidos, df_cambios = actualizar_estado_vencido(
            df_sin_vencidos, int(campaña_actual), int(año_actual), campaña_stock)
        ...
        return {"produciendo": path_produciendo, "base_descuentos": path_base_descuentos, "cambios": path_cambios}
    except Exception as e:
        logger.error(f"Error en Primer Produciendo: {e}", exc_info=True)
        raise