...
def ejecutar_listado_gral_en_proceso(parametros: dict) -> dict:
    return procesar_listado_gral_puro(**parametros)


class ListadoGralWindow(ctk.CTkFrame):
    ...
    def crear_interfaz(self):
        ...
        instrucciones = (
            "- Produciendo/Comprando: Archivos de campania a procesar.\n"
    ...
    def ejecutar_hilo(self):
        ...
        try:
            ...
            self.future_proceso = self.executor_proceso.submit(ejecutar_listado_gral_en_proceso, parametros)
            self.id_verificacion_after = self.after(150, self.verificar_estado_proceso)
        except Exception as error:
            logger.error("No se pudo iniciar Listado General: %s", str(error), exc_info=True)
